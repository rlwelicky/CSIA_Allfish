...
par(oma=c(0,0,0,0),mar=c(4, 3, 2, 2), family = 'sans')  #adjusting margins
...
SC <- read.csv("SC.csv") #importing data, I have each dataset as an individual CSV but you could do it as one
SC[is.na(SC)] <- 0
...
rasterImage(solepic, 0.5,11.5,3,10.5, interpolate = TRUE) #this adds the png to the plot -- you may need to play around with the numbers because it can skew teh dimensions depending on the png shape
